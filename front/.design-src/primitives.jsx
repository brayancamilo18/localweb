...
// Buttons
// ─────────────────────────────────────────────────────────────
function Btn({ children, kind = "primary", size = "md", icon, iconRight, fullWidth, disabled, style, ...rest }) {
  const sizes = {
  ...
  const s = sizes[size];
  const kinds = {
    primary: { bg: "var(--lw-accent)", color: "#fff", border: "transparent",
      shadow: "0 1px 0 rgba(255,255,255,.15) inset, 0 1px 2px rgba(15,23,42,.12)" },
    outline: { bg: "var(--lw-bg-elev)", color: "var(--lw-text)", border: "var(--lw-border)",
    ...
    dark:    { bg: "var(--lw-text)", color: "#fff", border: "transparent",
...
// Inputs
// ─────────────────────────────────────────────────────────────
function Field({ label, hint, error, counter, children, optional }) {
  return (
...
function Input({ value, placeholder, prefix, suffix, error, focus, disabled, type = "text", style }) {
  return (
    <div style={{
      ...
      boxShadow: focus ? "0 0 0 3px var(--lw-accent-ring)" : error ? "0 0 0 3px rgba(220,38,38,.12)" : "none",
      transition: "box-shadow .12s, border-color .12s",
      opacity: disabled ? .6 : 1,
      ...style,
    }}>
      {prefix && <span style={{ color: "var(--lw-text-3)", fontSize: 14 }}>{prefix}</span>}
      <input
        type={type} defaultValue={value} placeholder={placeholder} disabled={disabled}
        style={{
          flex: 1, border: "none", outline: "none", background: "transparent",
          fontFamily: "inherit", fontSize: 14, color: "var(--lw-text)",
          minWidth: 0,
        }}
      />
      {suffix && <span style={{ color: "var(--lw-text-3)", fontSize: 13 }}>{suffix}</span>}
    </div>
  );
}

function Textarea({ value, placeholder, rows = 4, focus, error, style }) {
  return (
    <textarea
      defaultValue={value} placeholder={placeholder} rows={rows}
      style={{
        padding: "10px 12px",
...
// Badge
// ─────────────────────────────────────────────────────────────
function Badge({ children, tone = "neutral", dot, icon, size = "md" }) {
  const tones = {
    ...
    pro:     { bg: "var(--lw-pro)", color: "#FFFBF5", border: "1px solid rgba(0,0,0,0.18)" },
...
function Logo({ size = 22, mono = false }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 8, color: "var(--lw-text)" }}>
      <span style={{
...
// Placeholder image
// ─────────────────────────────────────────────────────────────
function Placeholder({ ratio = "16:9", label, dark, h, w, style, children }) {
...
// Toggle / Switch
// ─────────────────────────────────────────────────────────────
function Switch({ checked, label, size = "md" }) {
  const w = size === "sm" ? 30 : 36, h = size === "sm" ? 18 : 22;
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 10 }}>
      <span style={{
        ...
        <span style={{
          position: "absolute", top: 2, left: checked ? w - h + 2 : 2,
          width: h - 4, height: h - 4, background: "#fff", borderRadius: 999,
          boxShadow: "0 1px 2px rgba(15,23,42,.2)", transition: "left .15s",
        }}/>
      </span>
      {label && <span style={{ fontSize: 13, color: "var(--lw-text-2)" }}>{label}</span>}
    </span>
  );
}
...
// Segmented control
// ─────────────────────────────────────────────────────────────
function Segmented({ options, value, size = "md", style }) {
  ...
  return (
    <div style={{
      ...
      {options.map((o) => {
        ...
        return (
          <span key={typeof o === "string" ? o : o.value}
            style={{
...
// Mini map (SVG fake)
// ─────────────────────────────────────────────────────────────
function MiniMap({ h = 140, pin = true, style }) {
  return (
    <div style={{
...
Object.assign(window, {
  Icon, Btn, Field, Input, Textarea, Badge, Logo, Placeholder,
  Card, Switch, Segmented, MiniMap, BrowserChrome,
});
