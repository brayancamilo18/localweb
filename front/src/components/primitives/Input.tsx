...
export default function Input({ label, error, helper, id, style, ...props }: InputProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {label ? (
        <label htmlFor={inputId} style={{ fontSize: 'var(--lw-form-label)', fontWeight: 500, color: 'var(--lw-text)' }}>
          {label}
        </label>
      ) : null}
      <input
      ...
      />
      {error ? <div style={{ fontSize: 'var(--lw-form-caption)', color: 'var(--lw-danger)' }}>{error}</div> : helper ? <div style={{ fontSize: 'var(--lw-form-caption)', color: 'var(--lw-text-3)' }}>{helper}</div> : null}
    </div>
  )
}
