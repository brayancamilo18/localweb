...
<style>
  ...
  *{ box-sizing:border-box; margin:0; padding:0; }
  ...
  @media (min-width:981px){
    .hero-bg::after{
      background:
        radial-gradient(ellipse at center, rgba(0,0,0,.35) 0%, rgba(0,0,0,.62) 55%, rgba(0,0,0,.92) 100%),
        linear-gradient(180deg, rgba(10,10,10,.58) 0%, rgba(10,10,10,.28) 32%, rgba(10,10,10,.82) 100%);
    }
  }
  ...
  /* Galería masonry (grid: fiable en iOS con muchas fotos; column-count falla en Safari) */
  .gallery{
    display:grid;
  ...
  @media (hover:none),(pointer:coarse){
    #galleryLive.reveal-up{
      opacity:1 !important;
      transform:none !important;
    }
    .gallery .photo img{ filter:none; -webkit-transform:none; transform:none; }
  ...
  /* Galería slider (modo Pro) — desactivado: siempre masonry/grid */
...
function noirClampFocal(value, fallback) {
  var n = Number(value);
  if (!isFinite(n)) return fallback;
  return Math.max(0, Math.min(100, n));
}

function noirApplyHeroFocal(heroBg, raw) {
  if (!heroBg) return;
  var fx = noirClampFocal(raw && raw.portada_focal_x, 50);
  var fy = noirClampFocal(raw && raw.portada_focal_y, 50);
  heroBg.style.setProperty('--hero-focal-x', fx + '%');
  heroBg.style.setProperty('--hero-focal-y', fy + '%');
}

function noirSetHeroCoverSrc(heroBg, heroMedia, src) {
  if (!heroMedia) return;
  if (!src) {
...
function updateNoirHeroSlider(raw) {
  ...
  noirSetHeroCoverSrc(heroBg, heroMedia, withCacheBust);
}
...
function updateNoirGallerySlider() {
